...
size_t io_blocksize(int fd) {
    ...
    size_t base_size = (size_t)lcm(page_size, fs_block_size);
    return base_size * OPTIMAL_MULTIPLIER;
}

void *aligned_allocate_buffer(size_t size) {
    void *memptr;
    long page_size = get_page_size();
    int ret = posix_memalign(&memptr, page_size, size);
    if (ret != 0) {
...
int main(int argc, char *argv[]) {
    ...
    char *buffer = (char*)aligned_allocate_buffer(buf_size);
    if (buffer == NULL) {
        close(fd);
        exit(EXIT_FAILURE);
    }
